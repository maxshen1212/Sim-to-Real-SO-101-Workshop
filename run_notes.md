...
**要指令請看 [run_cheatsheet.md](run_cheatsheet.md)。** 這裡是同一條 pipeline 的
**原理、陷阱、出錯怎麼辦** —— 每個警告都是實際踩過才寫下來的,不要當成一般說明刪掉。
...
>
> **校正檔統一放 `~/sim2real/Sim-to-Real-SO-101-Workshop/calibration/`**(git 追蹤),sim 與真機讀同一份。
